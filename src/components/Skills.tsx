...
const Skills = () => {
  ...
  const togleShow = () => {
    if (!showAll) {
      setMaxItems(groupedSkills.length);
    } else {
      setMaxItems(5);
    }
  ...
  return (
    <section id="skills" className="py-16 bg-gray-900 text-white">
      <div className="px-4">
        ...
        <p className="text-orange-500 py-4 cursor-pointer" onClick={togleShow}>
          {showAll ? "Show less" : "Show all"}
        </p>
        <div>
          {groupedSkills.slice(0, maxItems).map((grouped, index) => (
            <motion.div
              key={grouped.category + index}
              variants={cardVariants}
              initial="hidden"
              whileInView="visible"
              viewport={{ once: true }}
            >
              <p className="py-3 text-center bg-white text-[red] font-bold text-xl my-5">{grouped.category}</p>
